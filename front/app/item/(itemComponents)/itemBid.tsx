...
//     items[0]?.bids[items[0]?.bids.length - 1]?.bidAmount || items[0]?.price
//   );
...
//     socket.on("receive_bid", handleNewBidFromOthers);
//     socket.on("notification", () => {
//       console.log("test soket from back");
//     });
//     return () => {
//       socket.off("receive_bid", handleNewBidFromOthers);
//     };
//   }, []);
...
//     if (quant > lastBidAmount && quant <= items[0]?.price) {
//       // Send the bid to the server using socket
//       socket.emit("place_bid", { bidAmount: quant });
...
const ItemBid = ({ items }) => {
  ...
  useEffect(() => {
    ...
    const joinNewRoom = (newRoomId) => {
      // Emit leaveRoom event to disconnect from the current room
  ...
  }, [items]); // Add 'items' to the dependency array to trigger the effect when 'items' change

  console.log("qqqqqqqqqqqqqqqq", quant);
  ...
  // Function to adjust the bid quantity
  const adjustQuant = (amount) => {
  ...
  const handleSubmit = (e) => {
    ...
    // Send the bid data to the server using socket
    let test = socket.emit("placeBid", {
      userId: userId,
      itemId: itemId,
      bidAmount: quant,
    });
