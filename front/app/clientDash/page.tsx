...
const Dashboard = () => {
  ...
  const add = () => {
    axios
      .put(`http://localhost:5000/client/update/${id}`, {
        name: fname,
        lastName: lname,
        email,
        newPass: newPass,
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:5000/items/itemsBided/${id}`)
      .then((r) => {
        setCars(r.data);
      })
      .catch((err) => console.log(err));
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:5000/bid/fetch-items/${id}?page=${page}`)
      .then((r) => setBids((prev) => [...prev, ...r.data]))
      .catch((err) => console.log(err));
  }, [page]);
  useEffect(() => {
    axios
      .get(`http://localhost:5000/items/items-winner/${id}`)
      .then((r) => setEnded(r.data))
      .catch((err) => console.log(err));
  }, []);
    ...
    <div>
      ...
      <div className="grid mb-[10%] gap-[30px] grid-cols-[411px,auto] w-[90%] ml-[5%] mt-[5%]">
        <div className=" shadow-2xl h-fit">
          ...
          >
            {" "}
            <h1 className="p-3 font-[400] text-[20px]">Account Details</h1>
          </div>
          <div
            onClick={() => {
          ...
          >
            {" "}
            <h1 className="p-3 font-[400] text-[20px]">My Auction Bids</h1>
          </div>
          <div
            onClick={() => {
          ...
          >
            {" "}
            <h1 className="p-3 font-[400] text-[20px]">My Auction Activity</h1>
          </div>
          <div
            onClick={() => {
              localStorage.clear();
        ...
        {data[0] && (
          <div>
            ...
            <h1 className="font-[600] text-[30px] mb-[3%]">
              Auction Quick Links
            </h1>
        ...
        {data[1] && (
          <div>
            ...
            <div>
              ...
              <button
                onClick={() => add()}
                className="text-white font-[800] mt-[1%] bg-[#ff2800] rounded w-[132px] h-[43px] float-right hover:text-black hover:bg-white hover:border-[2px] hover:border-black hover:transition ease-in-out delay-50 ml-[15%]"
              >
                Save
              </button>
            </div>
                ...
                cars.map((el) => (
                  <div className=" w-[23%] h-[30%] rounded-3xl mb-[2%] border-[2px]  shadow-2xl">
                    ...
                    <div className="p-[15px] text-[#333333]">
                      <h1
                        className="cursor-pointer hover:text-[#ff2800] text-[20px] font-[600]"
                      ...
                      <h1 className="font-[300] text-[13px]">
                        {Math.floor(
                          (new Date(el.timeEnd) - new Date() > 0
                            ? new Date(el.timeEnd) - new Date()
                            : 0) / 3600000
                        )}
                        h
                      </h1>
              ...
              {ended?.map((el) => (
                <div className=" w-[23%] h-[30%] rounded-3xl mb-[2%] border-[2px]  shadow-2xl">
                  ...
                  <div className="p-[15px] text-[#333333]">
                    ...
                    <h1 className="font-[300] text-[13px]">Auction</h1>
                  </div>
                </div>
              ))}
            </div>
        ...
        {data[3] && (
          <div>
            <h1 className="font-[800] text-[20px] mb-[3%]">
              My Auction Activity
            </h1>
            <div className="w-full flex font-[600] text-center">
              <div className="w-[33%] h-[40px] border-[1px] border-gray-200">
                Date
              </div>
              <div className="w-[33%] h-[40px] border-[1px] border-gray-200">
                Auction
            ...
            {bids.map((el) => (
              <div className="w-full flex font-[600] bg-[#e5f2e5]">
                <div className="w-[33%] h-[40px] border-[1px] border-gray-200 flex justify-center items-center">
                  {el.createdAt && el.createdAt}
                </div>
                <div
                  className="w-[33%] h-[40px] border-[1px] border-gray-200 flex justify-center items-center text-[red] cursor-pointer"
                  onClick={() => router.push(`/item/${el.item.id}`)}
                >
                  {el.item.name && el.item.name}
                </div>
                <div className="w-[33%] h-[40px] border-[1px] border-gray-200 flex justify-center items-center">
                  {el.bidAmount && el.bidAmount}$
                </div>
              </div>
            ))}
            <button
              className="mt-[25px] bg-black text-white rounded w-[120px] h-[40px]"
